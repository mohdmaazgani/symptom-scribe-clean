...
export default function DocumentHub() {
  ...
  const handleFileUpload = async (files: FileList | File[]) => {
    ...
    try {
      ...
      const ocrResult = await extractTextFromDocument(file, (prog) => {
        setUploadProgress(10 + Math.round(prog * 0.4)); // 10% to 50%
      });

      setUploadProgress(60);
      setUploadStep("Encrypting & uploading document file...");
      ...
      if (storageError) {
        console.warn("Storage upload error (using local cache mode):", storageError);
      }
      ...
      if (dbError) {
        console.warn("Supabase database insert fallback to Dexie:", dbError);
      }

      // Always cache in Dexie
      await db.documents.put(newDocRecord);

      // Optionally trigger Edge Function to finalize OCR processing if needed
      try {
        await supabase.functions.invoke("process-document-ocr", {
          body: {
            documentId: newDocRecord.id,
            storagePath: newDocRecord.storage_path,
            extractedText: newDocRecord.extracted_text,
          },
        });
      } catch (edgeErr) {
        // Edge function failure is non-fatal since client-side OCR succeeded
        console.info("Edge OCR function completed/bypassed.");
      }

      setUploadProgress(100);
      setUploadStep("Upload & OCR complete!");
      showSuccess("Document Uploaded!", `Extracted ${ocrResult.text.length} characters of OCR text.`);
  ...
  const handleCopyText = (text: string) => {
    ...
    setCopied(true);
    showInfo("Copied to Clipboard", "Document OCR text copied.");
  ...
  const handleDownload = async (doc: DocumentRecord) => {
    try {
      ...
      if (error || !data) {
        showError("Download Failed", "Could not retrieve document file.");
        return;
      }
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 border-b border-slate-800/80 pb-6">
          <div>
            ...
            <p className="text-slate-400 text-sm mt-1">
              Upload, organize, and instantly search text across your lab reports, prescriptions, and medical records.
            </p>
          </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
          <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-sm">
            <p className="text-slate-400 text-xs font-medium uppercase tracking-wider">Total Vault Files</p>
          ...
          <div className="p-4 rounded-xl bg-slate-900/60 border border-slate-800/80 backdrop-blur-sm">
            <p className="text-slate-400 text-xs font-medium uppercase tracking-wider">Vault Storage</p>
        ...
        >
          ...
          <div className="flex flex-col md:flex-row items-center justify-between gap-6">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h3 className="text-lg font-semibold text-white">Upload New Medical Document</h3>
                <p className="text-slate-400 text-xs mt-0.5">
                  Drag & drop your lab report, prescription, or insurance image/PDF here, or browse files.
                </p>
              </div>
            </div>
            ...
            <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
              <select
                ...
                className="bg-slate-950 border border-slate-800 text-slate-200 text-xs rounded-lg px-3 py-2 focus:ring-1 focus:ring-cyan-500 outline-none"
              >
                <option value="lab_report">Category: Lab Report</option>
                <option value="prescription">Category: Prescription</option>
                <option value="insurance">Category: Insurance Paper</option>
                <option value="medical_record">Category: Medical History</option>
                <option value="other">Category: General</option>
              </select>

              <Button
                onClick={() => fileInputRef.current?.click()}
                disabled={isUploading}
                className="bg-cyan-600 hover:bg-cyan-500 text-white font-medium text-xs px-5 py-2.5 rounded-lg shadow-lg shadow-cyan-600/20"
              >
                Select File
              </Button>
            </div>
          ...
          <div className="mt-4 pt-4 border-t border-slate-800/60 flex flex-wrap items-center gap-2">
            ...
            <div className="flex items-center gap-1">
              <Input
                ...
                className="h-7 w-40 text-xs bg-slate-950 border-slate-800 text-slate-200 focus:ring-cyan-500"
              />
        ...
        <div className="flex flex-col md:flex-row items-center justify-between gap-4 bg-slate-900/60 border border-slate-800/80 rounded-2xl p-4 backdrop-blur-sm">
          ...
          <div className="relative w-full md:w-96">
            ...
            <Input
              ...
              placeholder="Search extracted OCR text, file names, tags..."
              className="pl-10 bg-slate-950 border-slate-800 text-slate-100 placeholder:text-slate-500 focus:border-cyan-500 text-xs rounded-xl"
            />
            {searchQuery && (
              <X
        ...
        {loading ? (
          <div className="py-20 text-center text-slate-400 text-sm">Loading your vault documents...</div>
          ...
          <div className="py-16 text-center border border-dashed border-slate-800 rounded-2xl bg-slate-900/20">
            ...
            <p className="text-slate-500 text-xs mt-1 max-w-sm mx-auto">
              {searchQuery
                ? `No documents match your query "${searchQuery}". Try a different keyword.`
                : "Upload your first medical report or prescription to start searching OCR text."}
            </p>
          </div>
        ) : viewMode === "grid" ? (
                ...
                >
                  <div>
                    ...
                    <div className="flex items-center justify-between gap-2 mb-3">
                      <Badge className="bg-slate-800 text-cyan-300 text-[10px] uppercase font-semibold border-slate-700">
                    ...
                    <div className="bg-slate-950/80 rounded-xl p-3 border border-slate-800/60 mb-4 min-h-[70px]">
                      <div className="flex items-center justify-between text-[10px] text-slate-500 mb-1">
                        ...
                        <span>{doc.extracted_text ? `${doc.extracted_text.length} chars` : "No Text"}</span>
                      </div>
                      <p className="text-xs text-slate-300 line-clamp-3 italic">
                        {doc.extracted_text || "No text extracted from this document file."}
                      </p>
                    </div>
                  </div>
          ...
          /* List View */
          <div className="bg-slate-900/60 border border-slate-800/80 rounded-2xl divide-y divide-slate-800/80 overflow-hidden">
            {filteredDocuments.map((doc) => (
              <div key={doc.id} className="p-4 hover:bg-slate-800/30 flex items-center justify-between gap-4 transition-colors">
                ...
                <div className="flex items-center gap-2 shrink-0">
                  ...
                  >
                    <Eye className="h-3.5 w-3.5 mr-1" /> Inspect OCR
                  </Button>
                  <Button
                    onClick={() => handleDownload(doc)}
                    variant="ghost"
              ...
              <>
                <DialogHeader>
                  ...
                  <DialogDescription className="text-xs text-slate-400">
                    Category: <span className="capitalize text-cyan-400">{activeDoc.category.replace("_", " ")}</span> • Uploaded on {new Date(activeDoc.created_at).toLocaleString()}
                  </DialogDescription>
                </DialogHeader>
                ...
                <div className="space-y-4 my-2">
                  ...
                  <div>
                    ...
                    <div className="p-4 rounded-xl bg-slate-900/90 border border-slate-800 text-slate-200 text-xs font-mono whitespace-pre-wrap max-h-60 overflow-y-auto leading-relaxed">
                      {activeDoc.extracted_text || "No OCR text extracted from this document."}
                    </div>
                  </div>
                  ...
                  {activeDoc.tags && activeDoc.tags.length > 0 && (
                    <div>
                      <span className="text-xs text-slate-400 block mb-1">Tags</span>
                      <div className="flex flex-wrap gap-1.5">
                        {activeDoc.tags.map((t) => (
                          <Badge key={t} className="bg-slate-800 text-cyan-300 text-xs">
                            #{t}
                          </Badge>
                        ))}
                      </div>
                ...
                <div className="flex items-center justify-end gap-3 border-t border-slate-800/80 pt-4 mt-2">
                  ...
                  <Button
                    onClick={() => handleDeleteDocument(activeDoc.id, activeDoc.storage_path)}
                    variant="destructive"
                    size="sm"
                    className="text-xs"
                  >
