...
export interface OxygenSaturationEntry { id: string; value: string; notes: string; timestamp: string; }

export const useOxygenSaturation = () => {
  const [inputValue, setInputValue] = useState("");
  const [notes, setNotes] = useState("");
  const [entries, setEntries] = useState<OxygenSaturationEntry[]>([]);
