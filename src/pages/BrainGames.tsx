...
import {
  ...
  Shield, Flame, Sparkles, Timer, RefreshCw
} from "lucide-react";
...
const BrainGames = () => {
  ...
  useEffect(() => {
    if (patternStreak >= 3 && patternStreak < 5) {
      ...
      toast({ title: "On Fire!", description: `${patternStreak} correct answers in a row!` });
    ...
    if (newFlipped.length === 2) {
      ...
      if (memoryCards[first] === memoryCards[second]) {
        ...
        if (newMatched.length === memoryCards.length) {
          ...
          showSuccess("Match Found!", `You matched a pair! (${newMatched.length / 2}/${memoryCards.length / 2})`);
        }
      } else {
        setTimeout(() => showWarning("No match", "Try again!"), 500);
  ...
  const generateTrendQuestion = (): TrendQuestion => {
    ...
    const patterns = [
      { name: "increasing", getValue: (i: number, start: number) => start + (i * 5), description: "Increasing by 5 each day" },
      { name: "increasing_fast", getValue: (i: number, start: number) => start + (i * 10), description: "Increasing by 10 each day" },
      { name: "decreasing", getValue: (i: number, start: number) => start - (i * 5), description: "Decreasing by 5 each day" },
      { name: "decreasing_fast", getValue: (i: number, start: number) => start - (i * 10), description: "Decreasing by 10 each day" },
      { name: "multiply", getValue: (i: number, start: number) => start * Math.pow(2, i), description: "Doubling each day" },
      { name: "alternating", getValue: (i: number, start: number) => i % 2 === 0 ? start : start + 15, description: "Alternating between two values" }
    ];
  ...
  const handlePatternAnswer = (answer: number) => {
    ...
    if (isCorrect) {
      ...
      setPatternScore(prev => prev + pointsEarned);
      setPatternStreak(prev => prev + 1);
      setXp(prev => prev + xpEarned);
      showSuccess("Correct!", `+${pointsEarned} points! Streak: ${patternStreak + 1}`);
    } else {
      setPatternStreak(0);
      showError("Incorrect", isTimeout ? "Time's up!" : `The correct pattern was: ${currentQuestion.patternDescription}`);
    }
          ...
          <CardContent className="space-y-6 text-center">
            <div className="p-8 bg-gradient-to-br from-primary/10 to-primary-glow/10 rounded-xl">
              ...
              <div className="mt-4 flex items-center justify-center gap-4">
                <div className="text-center"><p className="text-2xl font-bold text-primary">{level}</p><p className="text-xs text-muted-foreground">Level</p></div>
                <div className="text-center"><p className="text-2xl font-bold text-primary">{xp}</p><p className="text-xs text-muted-foreground">Total XP</p></div>
              </div>
            </div>
        ...
        <CardHeader>
          <CardTitle className="flex items-center justify-between flex-wrap gap-2">
            ...
            <div className="flex gap-2 items-center flex-wrap">
              <div className="flex items-center gap-2 bg-primary/10 px-3 py-1 rounded-lg"><Trophy className="w-4 h-4" /><span className="font-bold">{patternScore}</span></div>
              <div className={`flex items-center gap-2 px-3 py-1 rounded-lg ${patternStreak >= 3 ? 'bg-orange-500/20 text-orange-500' : 'bg-secondary/10'}`}><Flame className="w-4 h-4" /><span className="font-bold">{patternStreak}</span></div>
              <div className="flex items-center gap-2 bg-purple-500/10 px-3 py-1 rounded-lg"><Sparkles className="w-4 h-4 text-purple-500" /><span className="font-bold">Lv.{level}</span></div>
              <div className="flex items-center gap-2 bg-accent/10 px-3 py-1 rounded-lg"><Brain className="w-4 h-4" /><span className="font-bold">Q{questionsAnswered + 1}/{TOTAL_QUESTIONS}</span></div>
              <Button variant="outline" size="sm" onClick={resetPatternGame} className="gap-1"><RefreshCw className="w-4 h-4" /> Reset</Button>
          ...
          <CardDescription className="flex items-center justify-between">
            ...
            <div className="flex gap-2">
              <Button variant={timedMode ? "default" : "outline"} size="sm" onClick={() => setTimedMode(!timedMode)} className="gap-1"><Timer className="w-3 h-3" />{timedMode ? "Timed" : "Casual"}</Button>
              <Button variant="outline" size="sm" onClick={useFiftyFifty} disabled={lifelineUsed || showPatternFeedback} className="gap-1"><Shield className="w-3 h-3" />50-50 {lifelineUsed && "✓"}</Button>
            </div>
          </CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          {timedMode && !showPatternFeedback && (
            <div className={`flex items-center justify-center gap-2 p-3 rounded-lg ${questionTimeLeft <= 5 ? 'bg-red-500/20 text-red-500 animate-pulse' : 'bg-primary/10'}`}>
              <Timer className="w-5 h-5" /><span className="text-2xl font-bold">{questionTimeLeft}s</span><span className="text-sm text-muted-foreground">remaining</span>
            </div>
          )}
          <div className="p-6 bg-accent/30 rounded-xl">
            ...
            <div className="flex items-end justify-center gap-6 h-48 mb-6">
              ...
              <div className="flex flex-col items-center gap-2">
                <div className="w-16 bg-muted rounded-t-lg flex items-center justify-center" style={{ height: "80px" }}><span className="text-3xl text-muted-foreground">?</span></div>
          ...
          <div className="grid grid-cols-2 gap-3">
            {displayOptions.map((option, idx) => (
              <Button key={idx} variant={showPatternFeedback && option === currentQuestion.correctAnswer ? "default" : "outline"} className="text-lg py-6" onClick={() => handlePatternAnswer(option)} disabled={showPatternFeedback}>
          ...
          {showPatternFeedback && (
            <div className={`p-4 rounded-lg text-center ${isPatternCorrect ? "bg-green-500/10 border border-green-500" : "bg-red-500/10 border border-red-500"}`}>
              <p className={isPatternCorrect ? "text-green-600" : "text-red-600"}>
                {isPatternCorrect ? `Correct! Day 4 should be ${currentQuestion.correctAnswer} ${getMetricUnit(currentQuestion.metricType)}` : `Incorrect! The pattern was: ${currentQuestion.patternDescription}`}
              </p>
            </div>
          )}
        </CardContent>
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-bold text-foreground flex items-center gap-3"><Brain className="w-8 h-8 text-primary" />Brain Fitness Center</h1><p className="text-muted-foreground mt-2">Exercise your mind with fun, health-themed cognitive games</p></div>
      {!activeGame ? (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {games.map((game) => {
            const Icon = game.icon;
            return (
              <Card key={game.id} className="group hover:shadow-glow transition-all duration-300 cursor-pointer border-2" onClick={() => {
                ...
                <CardHeader>
                  <div className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${game.color} flex items-center justify-center mb-4 group-hover:scale-110 transition-transform`}><Icon className="w-8 h-8 text-white" /></div>
                  <CardTitle className="text-xl">{game.name}</CardTitle>
                  <CardDescription>{game.description}</CardDescription>
                </CardHeader>
                <CardContent><Button className="w-full">Play Now</Button></CardContent>
              </Card>
            );
          })}
        </div>
        ...
        memoryGameWon ? (
          <Card>
            <CardHeader><CardTitle className="text-center">You Won!</CardTitle><CardDescription className="text-center">You matched all pairs — great memory!</CardDescription></CardHeader>
            <CardContent className="space-y-6 text-center">
              <div className="p-8 bg-gradient-to-br from-primary/10 to-primary-glow/10 rounded-xl"><Trophy className="w-16 h-16 mx-auto text-primary mb-4" /><p className="text-2xl font-bold mb-2">All 4 pairs matched!</p></div>
              <div className="flex gap-4"><Button onClick={resetMemoryGame} className="flex-1 gap-2"><RefreshCw className="w-4 h-4" />Play Again</Button><Button variant="outline" onClick={() => setActiveGame(null)} className="flex-1">Back to Games</Button></div>
            </CardContent>
          </Card>
        ) : (
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center justify-between"><span>Memory Match Game</span><div className="flex gap-2"><div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-lg"><Trophy className="w-5 h-5 text-primary" /><span className="font-bold">{matchedCards.length / 2} / {memoryCards.length / 2} pairs</span></div><Button variant="outline" size="sm" onClick={resetMemoryGame} className="gap-1"><RefreshCw className="w-4 h-4" />Reset</Button><Button variant="outline" onClick={() => { setActiveGame(null); showInfo("Game Exited", "Come back anytime to play again!"); }}>Exit</Button></div></CardTitle>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between"><span>Quick Math Challenge</span><div className="flex gap-2 items-center"><div className="flex items-center gap-2 bg-primary/10 px-4 py-2 rounded-lg"><Trophy className="w-5 h-5 text-primary" /><span className="font-bold">{mathScore}</span></div><Button variant="outline" size="sm" onClick={() => { setMathScore(0); generateMathQuestion(); showSuccess("Game Reset!", "Score cleared — start fresh!"); }} className="gap-1"><RefreshCw className="w-4 h-4" />Reset</Button><Button variant="outline" onClick={() => { setActiveGame(null); showInfo("Math Challenge Exited", `Your final score: ${mathScore}`); }}>Exit</Button></div></CardTitle>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center justify-between"><span>Word Recall Challenge</span><div className="flex gap-2"><Button variant="outline" size="sm" onClick={startWordGame} className="gap-1"><RefreshCw className="w-4 h-4" />Reset</Button><Button variant="outline" onClick={() => { if (wordTimeoutRef.current) clearTimeout(wordTimeoutRef.current); setActiveGame(null); showInfo("Word Game Exited", "Keep practicing your memory!"); }}>Exit</Button></div></CardTitle>
            <CardDescription>Memorize the words, then recall them in order</CardDescription>
          </CardHeader>
          <CardContent className="space-y-6">
            <div className="space-y-4">
              <div className="p-6 bg-accent rounded-xl">
                <div className="flex items-center justify-between mb-4"><h3 className="text-lg font-semibold">Words to memorize:</h3>{wordPhase === "memorize" && (<div className="flex items-center gap-2 px-3 py-1 rounded-full bg-primary/10 text-primary font-semibold"><Clock className="w-4 h-4" /><span>{timeLeft}s</span></div>)}</div>
              ...
              <div className="p-6 bg-muted rounded-xl">
                ...
                <div className="flex flex-wrap gap-2">
                  {healthWords.map((word, idx) => {
                    ...
                    return (<Button key={idx} variant="outline" disabled={alreadySelected} className={alreadySelected ? "opacity-30 cursor-not-allowed line-through" : ""} onClick={() => { if (wordPhase !== "recall") { showWarning("Wait!", "Memorize phase is still active"); return; } setUserSequence([...userSequence, { word, index: userSequence.length }]); }}>{word}</Button>);
                  })}
                </div>
              </div>
              <div className="p-6 bg-accent rounded-xl min-h-[80px]">
                <h3 className="text-lg font-semibold mb-1">Your sequence: <span className="ml-2 text-sm font-normal text-muted-foreground">({userSequence.length} / {wordSequence.length} words)</span></h3>
                <p className="text-xs text-muted-foreground mb-4">Drag to reorder • ✕ to remove</p>
                {userSequence.length === 0 ? (<p className="text-muted-foreground text-sm">Click words above to build your sequence</p>) : (
                  <div className="flex flex-wrap gap-3">
                    {userSequence.map((item, idx) => (
                      <div key={`${item.word}-${idx}`} draggable onDragStart={() => setDragIndex(idx)} onDragOver={(e) => { e.preventDefault(); setDragOverIndex(idx); }} onDrop={(e) => { e.preventDefault(); if (dragIndex === null || dragIndex === idx) { setDragIndex(null); setDragOverIndex(null); return; } const updated = [...userSequence]; const [moved] = updated.splice(dragIndex, 1); updated.splice(idx, 0, moved); setUserSequence(updated.map((w, i) => ({ ...w, index: i }))); setDragIndex(null); setDragOverIndex(null); }} onDragEnd={() => { setDragIndex(null); setDragOverIndex(null); }} className={`flex items-center gap-1 pl-3 pr-2 py-2 rounded-lg font-semibold cursor-grab active:cursor-grabbing select-none transition-all duration-150 ${dragIndex === idx ? "opacity-40 scale-95 ring-2 ring-primary" : ""} ${dragOverIndex === idx && dragIndex !== idx ? "ring-2 ring-primary scale-105 bg-primary text-primary-foreground" : "bg-secondary text-secondary-foreground"}`}>
                        <span className="text-muted-foreground mr-1 text-xs select-none">⠿</span><span className="text-xs opacity-60 mr-1">{idx + 1}.</span><span>{item.word}</span>
                        <button onClick={(e) => { e.stopPropagation(); setUserSequence(userSequence.filter((_, i) => i !== idx).map((w, i) => ({ ...w, index: i }))); }} className="ml-2 w-5 h-5 rounded-full bg-destructive/20 hover:bg-destructive hover:text-white text-destructive flex items-center justify-center transition-colors duration-150 text-xs font-bold leading-none" title={`Remove "${item.word}"`}>×</button>
                      </div>
                    ))}
                  </div>
                )}
      ...
      ) : activeGame === "pattern" ? renderPatternGame() : null}
      <Card className="bg-gradient-to-br from-primary/5 to-primary-glow/5 border-primary/20">
        <CardHeader><CardTitle className="flex items-center gap-2"><Award className="w-6 h-6 text-primary" />Benefits of Brain Training</CardTitle></CardHeader>
        <CardContent><ul className="space-y-2 text-muted-foreground"><li className="flex items-start gap-2"><span className="text-primary">✓</span>Improves memory and cognitive function</li><li className="flex items-start gap-2"><span className="text-primary">✓</span>Enhances problem-solving abilities</li><li className="flex items-start gap-2"><span className="text-primary">✓</span>Boosts concentration and focus</li><li className="flex items-start gap-2"><span className="text-primary">✓</span>May help prevent cognitive decline</li></ul></CardContent>
      </Card>
    </div>
  );
};
