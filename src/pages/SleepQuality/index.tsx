...
import { History, Plus, Trash2, BarChart2 } from "lucide-react";
    ...
    <div className="container mx-auto px-4 py-8 space-y-8 animate-fade-in">
      <div className="border-b pb-6">
        <h1 className="text-3xl font-bold tracking-tight text-foreground flex items-center gap-2">
          ...
          {t("sidebar.items.sleepQuality", "Sleep Quality Log")}
        </h1>
        <p className="text-muted-foreground mt-1">Record sleep hours, quality rating, and occurrences of snoring, gasping, or night sweats.</p>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <div className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Record your sleep quality log data</CardDescription>
            </CardHeader>
            <CardContent className="space-y-4">
              <div className="space-y-2">
                <label className="text-xs font-semibold text-muted-foreground uppercase block">Sleep Duration & Quality Rating</label>
                <Input
                  value={hook.inputValue}
                  onChange={(e) => hook.setInputValue(e.target.value)}
                  placeholder="e.g. 7.5 hours - Good (no disruptions)"
                />
              </div>
              <div className="space-y-2">
                ...
                <textarea
                  ...
                  className="w-full min-h-[80px] p-3 bg-background border rounded-md text-sm resize-none focus:ring-2 focus:ring-indigo-500 focus:outline-none"
                />
              </div>
              <Button
              ...
              >
                <Plus className="h-4 w-4 mr-1" /> Log Sleep Quality
              </Button>
            </CardContent>
          </Card>
        </div>
        ...
        <div className="lg:col-span-2 space-y-6">
          <SleepSummary entries={hook.entries} />
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg flex items-center gap-2">
                  <BarChart2 className="h-5 w-5" /> Sleep History
                </CardTitle>
              ...
              {hook.entries.length === 0 ? (
                ...
                <div className="space-y-3 max-h-[400px] overflow-y-auto">
                  {hook.entries.slice().reverse().map((entry) => (
                    <div key={entry.id} className="flex items-start justify-between p-3 border rounded-lg hover:border-indigo-500/20 transition-all bg-background">
